...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def mostRetweeted: Tweet = {
    ...
    else if(left.notEmpty)
      max(elem, left.mostRetweeted)
    else if(right.notEmpty)
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(p => google.contains(p.text.toList))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(p => apple.contains(p.text.toList))
